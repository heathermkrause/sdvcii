...
gpf_ppt %>% 
    ...
    kable(digits = 0,
          col.names = c("Round of Data Collection", "District", 
                        "Number of Local Breen Cows per Household (Average)", "Number of Local Breed Cows per Household (Median)"),
          caption = "Treatment Group at All Data Collection Rounds")
```
...
gpf_ppt %>% 
    filter(round %in% c('Jul 2014', 'Jul 2016')) %>%
    group_by(round, district_gpf, household_type) %>%
    summarise(avgcows = mean(number_cattle_breed_2, na.rm = TRUE),
              mediancows = median(number_cattle_breed_2, na.rm = TRUE)) %>%
    filter(!is.na(mediancows)) %>%
    kable(digits = 0,
          col.names = c("Round of Data Collection", "District", "Treatment/Control", 
...
```{r}
gpf_ppt <- gpf_ppt %>%
    mutate(mean_value_cattle_breed_1 = as.numeric(mean_value_cattle_breed_1),
           mean_value_cattle_breed_2 = as.numeric(mean_value_cattle_breed_2),
           median_value_cattle_breed_1 = as.numeric(median_value_cattle_breed_1),
           median_value_cattle_breed_2 = as.numeric(median_value_cattle_breed_2))

gpf_ppt %>% 
    filter(round %in% c('Jul 2014', 'Jul 2016')) %>%
    group_by(round, district_gpf, household_type) %>%
    summarise(avg1 = mean(mean_value_cattle_breed_1, na.rm = TRUE),
              median1 = median(median_value_cattle_breed_1, na.rm = TRUE),
              avg2 = mean(mean_value_cattle_breed_2, na.rm = TRUE),
              median2 = median(median_value_cattle_breed_2, na.rm = TRUE)) %>%
    kable(digits = 0,
          col.names = c("Round of Data Collection", "District", "Treatment/Control", 
                        "Value of Local Breed Cattle in BDT (Average)", "Value of Local Breed Cattle in BDT (Median)",
                        "Value of Cross Breed Cattle in BDT (Average)", "Value of Cross Breed Cattle in BDT (Median)"),
          caption = "Treatment and Control Groups at the First and Last Data Collection Rounds")
...
gpf_ppt %>% 
    ...
    summarise(avg1 = mean(number_cattle_breed_1 * mean_value_cattle_breed_1 +
                             number_cattle_breed_2 * mean_value_cattle_breed_2, na.rm = TRUE),
              median1 = median(number_cattle_breed_1 * median_value_cattle_breed_1 +
                             number_cattle_breed_2 * median_value_cattle_breed_2, na.rm = TRUE)) %>%
    kable(digits = 0,
          col.names = c("Round of Data Collection", "Treatment/Control", 
                        "Value of Herd in BDT (Average)", "Value of Herd in BDT (Median)"),
          caption = "Treatment and Control Groups at the First and Last Data Collection Rounds")
