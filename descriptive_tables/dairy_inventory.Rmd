...
* Missing collection point data (liters) for round 1 control? Other bits of information that end up `NA` here and there?
* Lot of households that have zero production (zero daily production or `NA` recorded for all of their cows). Should I remove the zeroes/`NA` values before taking the median?
* Births per year table -- what is going on? 2 births per year? what should we do?
* There is no market linkage data for round 5, that I can find.
...
df <- bind_rows(gpf_ppt %>% 
              ...
              group_by(district_gpf, household_type, entry_id) %>% 
              summarize(cows = n(),
                        births = 2 * max(how_many_calf_born)) %>% 
              mutate(deo_id = entry_id,
                     round_month = 'Jul 2014'),
          gpf_ppt %>% 
              filter(round_month == 'Jul 2016') %>% 
...
df %>%
    kable(digits = 1,
          col.names = c("Round of Data Collection", "District", "Treatment/Control", 
                        "Median Number of Births per Female Adult Cow"),
          caption = "Treatment and Control Groups at the First and Last Data Collection Rounds")

ggplot(df, aes(round_month, rate, fill = household_type)) +
    geom_bar(stat = "identity", position = "dodge") +
    facet_wrap(~district_gpf) +
    labs(x = NULL, y = "Median Number of Births per Female Adult Cow") +
    theme(legend.title=element_blank())
