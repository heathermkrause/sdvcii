...
control_round5 <- left_join(read_excel("../data/SDVCII HH Survey_Excel Version_OSS_05092016.xlsx",
    ...
    left_join(read_excel("../data/SDVCII HH Survey_Excel Version_OSS_05092016.xlsx", 
              ...
              by = "DEO ID") %>%
    left_join(read_excel("../data/SDVCII HH Survey_Excel Version_OSS_05092016.xlsx", 
                         sheet = 6, col_types = c("numeric", rep("text", 35))),
              by = "DEO ID")
...
treatment_round1_cows <- read_excel("../data/GPF 1 Cows Long Correct.xlsx",
                                    sheet = 1)
```
...
make_names_consistent <- function(df) {
    names(df) <- names(df) %>%
        ...
        colnames(df)[str_detect(names(df), "neighbours")] <- "neighbors"
        ...
        colnames(df)[str_detect(names(df), 
                                "have_you_had_any_other_income_income_during_last_6_months")] <- "income_from_any_other_income_source_taka"
        colnames(df)[str_detect(names(df), 
                                "how_are_decision_taken_in_group")] <- "how_are_decisions_taken_in_group"
...
colnames(control_round5)[95] <- "total_score_knowledge"
colnames(control_round5)[101] <- "total_score_practice"
...
treatment_round5 <- treatment_round5 %>%
    select(-my_name_is,
           ...
           -sex_2, -sex_3) %>%
    rename(skill_level = skill_level_vet_doctor,
           knowledge_level = knowledge_level_vet_doctor,
           affected_by_disease_2 = affected_by_disease_yes_or_no_2,
           ...
           affected_by_disease_9 = affected_by_disease_total_9,
           ...
           vaccinated_9 = vaccinated_total_9,
           ...
           quality_products_ku = how_would_you_rate_quality_products,
           quality_service_ku = how_would_you_rate_quality_service,
           do_you_think_milk_transactions_are_fair = do_you_think_milk_sales_transactions_are_fair,
           explain_transactions = if_no_describe,
           are_you_satisfied_with_methods_measurement_milk = are_you_satisfied_with_methods_measurement,
           explain_methods = if_no_describe_why,
           how_often_have_you_gone_to_ku_during_last_3_month = how_often_have_you_gone_to_ku,
           what_type_effective_action_taken_by_fls_for_group_development = what_type_effective_action_taken_by_fls)
...
Round 1 is structured quite differently than the rest of it, so we'll have to do this differently.
...
treatment_round1 <- treatment_round1 %>%
    rename(entry_id = entryid,
           ...
           no_family_members_total = nooffamilymembers,
           ...
           where_do_you_sell_milk_most_often = wheredoyousellmilk1,
           ...
           how_are_decisions_taken_in_group = howthedecisionsaretakeninthegroup1,
    ...
    mutate(how_often_have_you_gone_to_ku_during_last_3_month = 0.5 * as.numeric(howoftendidyougototheinputkushopinthepast6months),
           ...
           have_you_earned_any_income_during_last_6_months = 2 * as.numeric(haveanyincomefromthosecattleduringlast3months),
...
We are going to make the dataset have one row per cow. Let's work on this, using joins and such. First let's make the names of the cow variables consisten.

```{r}
make_cow_names_consistent <- function(df) {
    names(df) <- names(df) %>%
        tolower() %>%
        str_replace_all("[ ]{2,}", " ") %>%
        str_replace_all(" ", "_")
        ...
        colnames(df)[str_detect(names(df), "deo")] <- "deo_id"
        colnames(df)[str_detect(names(df), "bread_of_cow")] <- "cow_breed"
        colnames(df)[str_detect(names(df), "type_of_cow")] <- "cow_type"
        colnames(df)[str_detect(names(df), "value_of_cow_in_bdt")] <- "cow_value_bdt"
        colnames(df)[str_detect(names(df), "daily_production_of_cow")] <- "cow_daily_production"
        colnames(df)[str_detect(names(df), "no_of_calve")] <- "no_of_calving"
        colnames(df)[str_detect(names(df), "sl_no")] <- "serial_no"

    df    
}
```

Now we can apply this to the data frames.

```{r}

treatment_round2_cows <- treatment_round2_cows %>%
    make_cow_names_consistent()
treatment_round3_cows <- treatment_round3_cows %>%
    make_cow_names_consistent()
treatment_round4_cows <- treatment_round4_cows %>%
    make_cow_names_consistent()
treatment_round5_cows <- treatment_round5_cows %>%
    make_cow_names_consistent()
control_round5_cows <- control_round5_cows %>%
    make_cow_names_consistent()

```

Now we need to do round 1, but it is a bit different from the others.

```{r}

treatment_round1_cows <- treatment_round1_cows %>%
    transmute(producer_name = `@03aNameoftheRespondent`,
           sex_respondent = `@03bSexFemale1Male2Other3`,
           household_type = `@11HHtype`,
           producer_id = MemberCode,
           cow_sl = Index1,
           ...
           cow_breed = CowBreed,
           cow_type = CowType,
           cow_value_bdt = CowValue,
           cow_daily_production = CowProduction,
           last_calving_date = CowCalfDate,
           no_of_calving = CowCalfCount)
 
```


Now let's join each `cow` data frame to its producer data frame. Rounds 2 through 5 can be matched up via `deo_id`; we have to do round 1 by a combination of name, sex, and household type.
...
treatment_round2 <- left_join(treatment_round2_cows, 
                              treatment_round2, by = "deo_id") %>%
    select(-one_of(cow_columns), everything())
treatment_round3 <- left_join(treatment_round3_cows, 
                              treatment_round3, by = "deo_id") %>%
    select(-one_of(cow_columns), everything())
treatment_round4 <- left_join(treatment_round4_cows, 
                              treatment_round4, by = "deo_id") %>%
    select(-one_of(cow_columns), everything())
treatment_round5 <- left_join(treatment_round5_cows, 
                              treatment_round5, by = "deo_id") %>%
    select(-one_of(cow_columns), everything())
control_round5 <- left_join(control_round5_cows, 
                              control_round5, by = "deo_id") %>%
    select(-one_of(cow_columns), everything())

```

Now let's try round 1, which is trickier because there is no key linking the cows and producers.

```{r}
cow_columns <- c("cow_sl", "cow_identity", "cow_breed", 
                 "cow_type", "cow_value_bdt", "cow_daily_production",
                 "last_calving_date", "no_of_calving")

treatment_round1 <- treatment_round1 %>%
    mutate(sex_respondent = as.numeric(sex_respondent),
           household_type = as.numeric(household_type),
           producer_id = as.numeric(producer_id))

treatment_cows <- left_join(treatment_round1_cows %>%
                           filter(!is.na(producer_id)), 
                       treatment_round1 %>%
                           filter(!is.na(producer_id)) %>%
                           distinct(producer_id, .keep_all = TRUE), 
                       by = "producer_id") %>%
    select(-one_of(cow_columns), everything()) %>%
    mutate(producer_name = producer_name.y,
           sex_respondent = sex_respondent.y,
           household_type = household_type.y) %>%
    select(-producer_name.x, -producer_name.y,
           -sex_respondent.x, -sex_respondent.y,
           -household_type.x, -household_type.y)

control_cows <- left_join(treatment_round1_cows %>% 
                              filter(is.na(producer_id)),
                          treatment_round1 %>% 
                              filter(is.na(producer_id)) %>%
                              distinct(producer_name, sex_respondent, household_type, .keep_all = TRUE),
                          by = c("producer_name", "sex_respondent", "household_type")) %>%
        select(-one_of(cow_columns), everything()) %>%
    select(-producer_id.x, -producer_id.y)

```

This is not perfectly correct because a) there were duplicates in the producer ID in the treatment producers and b) the joining for the control producers had to be done on a combination of name, sex, and household type. About what level of mismatch is possibly being introduced?

```{r}
treatment_round1 %>% 
    filter(!is.na(producer_id)) %>% 
    select(producer_id) %>% 
    summarize(frac_distinct = n_distinct(producer_id)/n())

treatment_round1 %>% 
    filter(is.na(producer_id)) %>% 
    select(producer_name, sex_respondent, household_type) %>% 
    summarize(frac_distinct = n_distinct(producer_name, sex_respondent, household_type)/n())
```

So 99% of the cows in the treatment groups were matched based on producer ID and 88% of the cows in the control groups were matched based on a combination of name, sex, and household type.

Anyway, ONWARD.

```{r}
treatment_round1 <- bind_rows(treatment_cows, control_cows)    
```
