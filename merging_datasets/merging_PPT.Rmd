...
ppt3 <- bind_rows(ppt3_round1 %>% mutate(round = 1), 
    ...
    select(round, team_name, group_name, group_type, group_code, village, 
           union, union_code, upazila, upazila_code, 
           district, district_code, ward_number,
...
ppt3[,c(4:5,8,10,12:19,22:35)] <- lapply(ppt3[,c(4:5,8,10,12:19,22:35)], as.numeric)
...
ppt1_round1 <- ppt1_round1 %>%
    rename(team_name = Team,
           ...
           group_type = `Producer Type (SDVC I&II=1, SDVC II=2)`,
           producer_serial = Sl,
...
ppt1_round5 <- ppt1_round5 %>%
    rename(team_name = `Team Name`,
           ...
           sex = `Sex (W=1, M=2)`,
           group_leader = `Group Leader  (Yes=1; No=0)`,
           group_type = `Producer types (SDVC I&II=1; SDVC II=2)`,
...
ppt1 <- bind_rows(ppt1_round1 %>% mutate(round = 1), 
    ...
    select(round, team_name, group_name, group_type, group_code, group_code_old,
           ...
           evaluation_date, facilitator,
           producer_name, producer_id, producer_serial, producer_status,
           sex, guardian_name, group_leader,
           ...
           increased_volume, proper_milking, feed_after_milking, market_info,
           selling_dft, satisfied_dft, keeping_records, group_saving, 
...
## now do the conversion
ppt1[,c(4:6,13:22,26:27,29,31:63)] <- lapply(ppt1[,c(4:6,13:22,26:27,29,31:63)], as.numeric)
...
ppt <- bind_rows(ppt1 %>% mutate(data_type = 'ppt1'), 
    ...
    select(data_type, round, 
           team_name, group_name, group_type, group_code, group_code_old,
