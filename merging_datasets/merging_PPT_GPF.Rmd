...
Let's change the `group_code` and `round` to numeric in both data sets so we can join by them.
...
A WHOLE BUNCH OF GROUP CODES ARE MISSING IN GPF.

```{r}
mean(is.na(gpf$group_code))
```
...
THIS IS ALL FULL OF `NA` VALUES RIGHT NOW BECAUSE OF ALL THE MISSING GROUP CODE VALUES IN GPF.
